function CTableController(){
    ...
    this._initDeck = function(){
        ...
        for(var k=0;k<aCardInfo.length;k++){
            ...
            var oCard = new CCard(true,CARD_DECK,pStart,aCardInfo[k].frame,aCardInfo[k].rank,aCardInfo[k].suit,false,_oContainerDeck);
    ...
    this.endTurn = function(){
        ...
        if(_aCardDeck.length === 0){
            //CHECK IF THIS IS THE THIRD TIME THAT DECK IS EMPTY. IF SO MATCH IS OVER
            _iNumShuffle++;
            if(_iNumShuffle === MAX_NUM_SHUFFLE){
                s_oGame.gameOver(true);
            }else{
                ...
                for(var k=0;k<_aCardsInWastePile.length;k++){
                    ...
                    var oCard = new CCard(true,CARD_DECK,pStart,_aCardsInWastePile[k].getFrame(),_aCardsInWastePile[k].getValue(),_aCardsInWastePile[k].getSuit(),
                                                                                                                                _aCardsInWastePile[k].isWasteCard(),_oContainerDeck);
                    oCard.setClickable(false);
                    oCard.addEventListener(ON_CARD_ANIMATION_ENDING,s_oGame._onCardAnimEnd);
    ...
    this.addCardToWastePile = function(oCardToAdd){
        ...
        var oCard = new CCard(false,CARD_WASTE,new CVector2(_pWastePileOffset.x,_pWastePileOffset.y),oCardToAdd.getFrame(),oCardToAdd.getValue(),oCardToAdd.getSuit(),
                                                                                                                                        true,_oContainerDeck);
