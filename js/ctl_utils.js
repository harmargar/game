...
function sizeHandler() {
	...
	var destW = Math.round(CANVAS_WIDTH * multiplier);
        ...
        if(s_oSelectPanel !== null){
            s_oSelectPanel.refreshButtonPos();
        }
...
function fullscreenHandler(){
    ...
    if (s_oSelectPanel !== null){
        s_oSelectPanel.resetFullscreenBut();
    }
}
...
function printCards(aCards){
    console.log("START PRINT");
    for(var k=0;k<aCards.length;k++){
        console.log(k+"---->"+aCards[k].getValue() + " suit "+aCards[k].getSuit());
    }
