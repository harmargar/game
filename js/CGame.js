function CGame(){
    ...
    this.checkIfDroppingWasteCard = function(oCard){
        ...
        if(_oCardPickedFromWaste === null || !oCard.isWasteCard()){
            return false;
        }
    ...
    this._releaseWasteCard = function(oCard){
        if(_aPlayerHand[PLAYER_ID].isHighlight()){
            ...
            _oCardPickedFromWaste.setAsWasteCard(true);
    ...
    this.gameOver = function(bEndless){
        _bUpdate = false;
        if(bEndless){
            for(var i=0;i<_aPlayerHand.length;i++){ 
                var iFinalScore = this._calculateScore(i);
                _aPlayerHand[i].increaseScore(iFinalScore);
            }
        }else{
            var iFinalScore = 0;
            for(var i=0;i<_aPlayerHand.length;i++){
